...
class ActivationActivity : AppCompatActivity() {
    ...
    private fun runStatusContentCollapseSyncThenNavigate(phone: String?, code: String, deviceId: String, isTesting: Boolean) {
        ...
        // Defer collapse until utility card has finished layout (content swap causes resize; immediate animate gets cut short)
        id.utilityCard.post {
            if (isDestroyed || isFinishing) return@post
            ActivationAnimationHelper.runSuccessCollapseThenSync(
                id.utilityCard,
                id.activationInputCard,
                id.activationStatusCardContainer,
                collapseDurationMs = 400L,
                syncDurationMs = 800L,
                onComplete = {
                    if (isDestroyed || isFinishing) return@runSuccessCollapseThenSync
    ...
    private fun showActivationUI(isTransitioningFromSplash: Boolean = false) {
        ...
        // TEST: Keypad style switcher (REMOVE AFTER TESTING)
        setupKeypadStyleTestButton()
    ...
    // ===== TEST: Keypad Style Switcher (REMOVE AFTER TESTING) =====
    private val keypadStyles = listOf(
        "NEON_CYBER" to R.drawable.keypad_button_neon_cyber,
        "GLASS" to R.drawable.keypad_button_glass,
        "SOLID" to R.drawable.keypad_button_solid,
        "MINIMAL" to R.drawable.keypad_button_minimal,
        "ELEVATED" to R.drawable.keypad_button_elevated
    )
    private var currentKeypadStyleIndex = 0

    private fun setupKeypadStyleTestButton() {
        val testButton = id.root.findViewById<TextView>(R.id.keypadStyleTestButton) ?: return
        testButton.setOnClickListener {
            currentKeypadStyleIndex = (currentKeypadStyleIndex + 1) % keypadStyles.size
            val (styleName, drawableRes) = keypadStyles[currentKeypadStyleIndex]
            testButton.text = "TAP TO CHANGE KEYPAD STYLE: $styleName"
            applyKeypadStyle(drawableRes)
        }
    }

    private fun applyKeypadStyle(drawableRes: Int) {
        // Use KeypadHelper to apply style to all keys
        val textColor = if (drawableRes == R.drawable.keypad_button_solid) {
            resources.getColor(R.color.button_text_dark, theme)
        } else {
            resources.getColor(R.color.theme_primary, theme)
        }
        KeypadHelper.applyStyle(drawableRes, textColor)
    }
    // ===== END TEST =====
