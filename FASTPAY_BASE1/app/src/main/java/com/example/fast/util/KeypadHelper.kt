...
object KeypadHelper {
    ...
    private var onClearAll: (() -> Unit)? = null
    private var onPaste: ((String) -> Unit)? = null
     ...
     * @param onClearAll Optional callback when long-press backspace (clear all)
     * @param onPaste Optional callback when paste from clipboard
     */
    fun setup(
        rootView: View,
        numericKeypadId: Int,
     ...
     * Returns true if paste was successful, false if no valid content.
     */
    fun pasteFromClipboard(context: Context): Boolean {
        ...
        if (!text.isNullOrBlank()) {
            ...
            val filtered = text.filter { it.isLetterOrDigit() }.uppercase()
            if (filtered.isNotEmpty()) {
                onPaste?.invoke(filtered)
                return true
            }
        }
        return false
    ...
    private fun setupNumericKeypad(root: View) {
        ...
        // Backspace key - tap to delete one, long-press to clear all
        root.findViewById<View>(R.id.activationKeypadBackspace)?.apply {
            setOnClickListener { keyView ->
                ActivationAnimationHelper.keypadKeyPress(keyView)
                keyView.performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP)
                onBackspace?.invoke()
            }
            setOnLongClickListener { keyView ->
                ActivationAnimationHelper.keypadKeyPress(keyView)
                // Stronger haptic for clear all
    ...
    private fun setupAlphabetKeypad(root: View) {
        ...
        // Backspace key - tap to delete one, long-press to clear all
        root.findViewById<View>(R.id.keyAlphaBackspace)?.apply {
            setOnClickListener { keyView ->
                ActivationAnimationHelper.keypadKeyPress(keyView)
                keyView.performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP)
                onBackspace?.invoke()
