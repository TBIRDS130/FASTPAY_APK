...
object PermissionManager {
    ...
    /**
     * Build "to-request" list for current cycle: cycle 1 = all missing from input; cycle 2..N = only mandatory still missing.
     */
    private fun buildToRequestForCycle(activity: Activity, state: PermissionListState): List<String> {
        ...
        val mandatoryPerms = inputRequests.filter { it.isMandatory }.map { it.permission }.toSet()
        return when (state.currentCycle) {
            1 -> inputRequests.map { it.permission }.filter { !hasRuntimePermission(activity, it) }
            else -> mandatoryPerms.filter { !hasRuntimePermission(activity, it) }
        }
    }
    ...
    fun handleRequestPermissionListResult(
        ...
        val mandatoryStillDenied = state.inputRequests
            .filter { it.isMandatory }
            .any { !hasRuntimePermission(activity, it.permission) }
        if (mandatoryStillDenied && state.currentCycle < state.maxCyclesForMandatory) {
            state.currentCycle++
     ...
     * When the user returns to the app, call [onSpecialPermissionReturn] from onResume.
     *
     ...
     * Call [handleRequestPermissionListResult] from onRequestPermissionsResult with [PERMISSION_LIST_REQUEST_CODE].
     *
