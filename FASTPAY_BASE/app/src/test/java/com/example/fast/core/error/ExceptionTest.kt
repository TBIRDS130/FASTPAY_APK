package com.example.fast.core.error
...
class ExceptionTest {
    ...
    @Test
    fun `test FirebaseException permission error getUserMessage`() {
        val exception = FirebaseException.permissionError("writeData")
        assertThat(exception.getUserMessage()).contains("permission")
    }

    @Test
    fun `test FastPayException getErrorDetails includes cause when present`() {
        val cause = RuntimeException("root cause")
        val exception = FastPayException("Wrapper", cause, "ERR_1")
        assertThat(exception.getErrorDetails()).contains("ERR_1")
        assertThat(exception.getErrorDetails()).contains("RuntimeException")
    }
}
