...
1. **One-command test flow**
   From repo root: `.\scripts\test-build.ps1`
   - Builds debug APK
...
4. **Reproducible, shareable flow**
   ...
   - New tasks (e.g. “test activation”, “verify permission flow”) can be run by telling the agent to use `.\scripts\test-build.ps1` and the logcat filter above.
...
- **A script it can run** (`test-build.ps1`) so it can “test on device” when you ask (build + install + hint for logcat).
...
- **You:** Run `.\scripts\test-build.ps1` when you want a quick debug build + install and the logcat command.
- **You:** Ask the agent to “test on device” or “build debug and install”; it can use the same script and ENVIRONMENT.md.
- **Agent:** For testing tasks, run `.\scripts\test-build.ps1` from repo root and suggest `adb logcat -s ActivationActivity:D` to verify.
